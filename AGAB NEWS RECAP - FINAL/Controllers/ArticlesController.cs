    ...
    public class ArticlesController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (Request.Params.Get("search")!=null)
            {
                ...
                //Search in Categories (Cat Name)
                List<int> categories = db.Categories.Where(
                    at => at.CategoryName.Contains(search)
                    ).Select(a => a.CategoryId).ToList();

                List<int> categoriesIds = db.Articles.Where(
                    at => categories.Contains(at.CategoryId)
                    ).Select(a => a.Id).ToList();

                //Unique list of articles
                List<int> mergeIds = articlesIds.Union(changeIds).ToList();
                List<int> mergeIds2 = articlesIds.Union(chapterIds).ToList();
                List<int> mergeIds3 = articlesIds.Union(categoriesIds).ToList();

                //List of articles that contain the search string either in article title, content or comments
                articles = db.Articles.Where(article => mergeIds3.Contains(article.Id)).Include("Category").Include("User").OrderBy(a => a.Date);
